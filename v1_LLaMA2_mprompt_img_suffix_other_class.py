...
def parse_args():
    ...
    parser.add_argument("--class_img", type=str, default="S1")
...
#==============================================================#
Output_log_file_path = "./Result_other_class/" + args.class_img + "/output_" + str(class_tag) + '_'+ os.path.basename(args.img_path) + ".log"
...
if args.img_path.endswith('pt'):
    image_emb = torch.load(args.img_path).to(device)
else:
...
for p in range(model.test_num):
    print('-------------------------Response Goal ' + str(model.test_prompt_index[p]) + ':' + model.test_prompt[
        p] + '----------------------------------------')
