...
from minigpt4.conversation.conversation import Chat, CONV_VISION_Vicuna0, CONV_VISION_LLama2, StoppingCriteriaSub, CONV_LLama2
...
def nn_project(curr_embeds, embedding_layer, model):
    ...
    projected_embeds = model.model.embed_tokens(nn_indices)

    return projected_embeds, nn_indices


def prompt_generate(model,  embedding, step=10000):
    ...
    optimizer2 = AdamW([embedding], lr=0.5)  # usually lr = 0.5
    scheduler2 = get_linear_schedule_with_warmup(
        optimizer2,
        num_warmup_steps=500, num_training_steps=step)  # usually num_warmup_steps is 500
    ...
    for epoch in range(step):
        ...
        for p in range(2):
            ...
            if glb_step % 10 == 0 and actual_step % 1 == 0:
                projected_embeds, nn_indices = nn_project(embedding.squeeze(0),llama2_dict_emb, model)
                print("Front part prompt: ", model.model.llama_tokenizer.convert_ids_to_tokens(nn_indices))

            if 1 and actual_step % 1 == 0:
                ### COPY SOFT PROMPT ###
                soft_prompt_copy = embedding.clone().detach()
                ### PROJECT ###
                projected_embeds, nn_indices = nn_project(embedding.squeeze(0),llama2_dict_emb, model)
                projected_embeds = projected_embeds.unsqueeze(0)
                ### PROJECTED EMBEDDING FOR FORWARD ###
                embedding.data = projected_embeds.data

            tot_train_time -= time.time()

            inp = []
            inp.append(embedding)
            inp.append([i for i in range(0+p*b, 0+p*b+b)])
            #inp.append(p)
            ...
            if True:
                embedding.data = soft_prompt_copy.data
            ...
            '''
                ...
                beta_i = 1 * (beta_decay_rate ** (
                        glb_step - 1))  # note beta is 1 at step 1 and 0.0001 at 5001 if max steps is 5000; cause glb_step is incremented first
                ...
                # project
                projected_embeds, nn_indices = nn_project(prompt_model.template.soft_embeds,
                                                          prompt_model.template.raw_embedding)
...
#==============================================================#
Output_log_file_path = "./Result_new/" + class_tag + "/output" + "_PEZn" + ".log"
...
class MiniGPT(nn.Module):
    def __init__(self, class_tag):
        ...
        embeddings = torch.zeros(1,10,4096).to(device)
        image_list = []
        image_list.append(embeddings)
        ...
        for k in range(self.train_num):
            ...
            embs, inputs_tokens, _ = self.get_context_emb(conv_, image_list)
    ...
    def get_context_emb(self, conv, embedding):
        prompt = conv.get_prompt_text()
        ...
        self.model.llama_tokenizer.pad_token = self.model.llama_tokenizer.unk_token
        #input = tokenizer(prompts, padding='max_length', max_length=20, return_tensors="pt");

        output = self.model.llama_tokenizer(prompt, padding='max_length', max_length=200, return_tensors="pt", add_special_tokens=False).to(self.device)
        seg_tokens = output.input_ids
        mask = output.attention_mask
        atd_mask = torch.ones(1,10).to(self.device)
        mask = torch.cat((mask,atd_mask), dim=1)
        ...
        idx_start = 0
        for i in range(len(seg_tokens[0])):
            if (seg_tokens[0][i] == 0 and seg_tokens[0][i + 1] != 0):
                idx_start = i +1
                break
        ...
        inputs_tokens = []
        inputs_tokens.append(seg_tokens[0,:idx_start + 83].unsqueeze(0))
        #inputs_tokens.append( torch.from_numpy(np.ones((1,32))*(-200)).to(device) ) #for 224*224 num_Vtokens=32
        inputs_tokens.append(torch.from_numpy(np.ones((1, 10)) * (0)).to(self.device))  # for 448*448 num_Vtokens=256
        inputs_tokens.append(seg_tokens[0,idx_start + 83:].unsqueeze(0))
        ...
        seg_tokens = [seg_tokens[0,: idx_start +83].unsqueeze(0), seg_tokens[0, idx_start + 83:].unsqueeze(0)]

        seg_embs = [self.model.embed_tokens(seg_t) for seg_t in seg_tokens]

        mixed_embs = [emb for pair in zip(seg_embs[:-1], embedding) for emb in pair] + [seg_embs[-1]]
        mixed_embs = torch.cat(mixed_embs, dim=1)
        return mixed_embs, inputs_tokens, mask
    
    
    def get_context_emb2(self, conv, embedding):
        prompt = conv.get_prompt_text()
        ...
        '''
        # print(prompt_segs)
        seg_tokens = self.model.llama_tokenizer(prompt, return_tensors="pt", add_special_tokens=False).to(self.device).input_ids
            # only add bos to the first seg
        ...
        inputs_tokens = []
        inputs_tokens.append(seg_tokens[0,:83].unsqueeze(0))
        #inputs_tokens.append( torch.from_numpy(np.ones((1,32))*(-200)).to(device) ) #for 224*224 num_Vtokens=32
        inputs_tokens.append(torch.from_numpy(np.ones((1, 10)) * (-200)).to(self.device))  # for 448*448 num_Vtokens=256
        inputs_tokens.append(seg_tokens[0,83:].unsqueeze(0))
        ...
        # print(inputs_tokens)
        # print(inputs_tokens.shape)
        seg_tokens = [seg_tokens[0,:83].unsqueeze(0), seg_tokens[0,83:].unsqueeze(0)]
    ...
    def forward(self, inp):
        ...
        if 1:
            ...
            embs, _ , mask= self.get_context_emb(conv_, image_list)
            ...
            outputs = self.model.llama_model(attention_mask = mask, inputs_embeds=embs)
            ...
            print(loss_fct(shift_logits_.view(-1, shift_logits_.size(-1))[-25:], shift_labels_.view(-1)[-25:])
        ...
        if 1:
            ...
            for i in k:
                if i >= 15:
                    break
                conv_ = self.conv[i]
                target_len_ = self.target_len[i]
                shift_labels_ = self.shift_labels[i]

                embs, _, mask= self.get_context_emb(conv_, image_list)
                embeds.append(embs)
                sl.append(shift_labels_)
                masks.append(mask)

            from datasets import load_dataset, Dataset


            embs = torch.cat(embeds, dim=0)
            ...
            outputs = self.model.llama_model(attention_mask = masks,inputs_embeds=embs)
            logits = outputs.logits
            ...
            loss += loss_fct(shift_logits_.view(-1, shift_logits_.size(-1)), sl.view(-1))
...
llama2_dict_emb = torch.load('./dataset/llama2_dict_embeddings.pt').half()
...
embeddings = (torch.rand(1,10,4096)*2 -1).half().to(device)
embeddings = prompt_generate(model, embeddings, step = 1)
...
# 计算内积
dot_products = torch.matmul(embeddings, llama2_dict_emb.t().half())
...
for p in range(model.train_num):
    ...
    embs, _= model.get_context_emb2(q_conv, image_list)
...
for p in range(model.test_num):
    ...
    embs, _ = model.get_context_emb2(test_conv, image_list)
    print(embs.shape)
...
train_prompt_top1 = [adv_suffix_top1 + goal for goal in train_goals]
train_prompt_random_1 = [adv_suffix_random_1+ goal for goal in train_goals]
train_prompt_random_2 = [adv_suffix_random_2+ goal for goal in train_goals]
train_prompt_random_3 = [adv_suffix_random_3+ goal for goal in train_goals]
train_prompt_random_4 = [adv_suffix_random_4+ goal for goal in train_goals]
train_prompt_random_5 = [adv_suffix_random_5+ goal for goal in train_goals]

test_prompt_top1 = [adv_suffix_top1+ goal for goal in test_goals]
test_prompt_random_1 = [adv_suffix_random_1 + goal for goal in test_goals]
test_prompt_random_2 = [adv_suffix_random_2 + goal for goal in test_goals]
test_prompt_random_3 = [adv_suffix_random_3 + goal for goal in test_goals]
test_prompt_random_4 = [adv_suffix_random_4 + goal for goal in test_goals]
test_prompt_random_5 = [adv_suffix_random_5 + goal for goal in test_goals]
