...
from torchattacks.attacks.pgd_uap_test import *
...
args = parse_args()
cfg = Config(args)
#args.log_path = './Result_syy/random100/' + str(seed) + "output.log"
...
class MiniGPT(nn.Module):
    ...
    def get_context_emb(self, conv, img_list, flag):
        ...
        targets_segs = prompt_segs[1].split('[/INST]')
        
        target_tokens = self.model.llama_tokenizer(targets_segs[1], return_tensors="pt", add_special_tokens=False).to(self.device).input_ids
        
        #print(targets_segs[1])
        ...
        #print(seg_tokens[1].shape[1])
        
        inputs_tokens = []
        #inputs_tokens.append(seg_tokens[0])
        inputs_tokens.append(torch.from_numpy(np.ones((1, len(seg_tokens[0]))) * (-200)).to(self.device))
        # inputs_tokens.append( torch.from_numpy(np.ones((1,32))*(-200)).to(self.device) ) #for 224*224 num_Vtokens=32
        inputs_tokens.append(torch.from_numpy(np.ones((1, 64)) * (-200)).to(self.device))  # for 448*448 num_Vtokens=256
        inputs_tokens.append(torch.from_numpy(np.ones((1, seg_tokens[1].shape[1]-target_length)) * (-200)).to(self.device))
        inputs_tokens.append(seg_tokens[1][:,-target_length:])

        dtype = inputs_tokens[3].dtype
        inputs_tokens = torch.cat(inputs_tokens, dim=1).to(dtype)
        seg_embs = [self.model.embed_tokens(seg_t) for seg_t in seg_tokens]
    ...
    def forward(self, inp, ):
        ...
        if 1:
            ...
            #print(shift_labels_.view(-1))

            loss += loss_fct(shift_logits_.view(-1, shift_logits_.size(-1)), shift_labels_.view(-1))
            
            length = [17, 23, 19, 25, 20, 19, 21, 16, 15, 12]
            #length = [20]
            
            #print("prompt {}".format(i))
            loss2 = loss_fct(shift_logits_.view(-1, shift_logits_.size(-1))[-length[i]:], shift_labels_.view(-1)[-length[i]:])
            l += loss2
            #print(loss2)
            loss_token = []
            #for j in range(length[i]):
                #loss_token.append(loss_fct(shift_logits_.view(-1, shift_logits_.size(-1))[-length[i]+j], shift_labels_.view(-1)[-length[i]+j]).item())
            
            #print(loss_token)
            #writer.add_scalar("loss",loss,s)
            
            
            #if torch.argmax(shift_logits_.view(-1, shift_logits_.size(-1))[-length[i]]) == 18585:
                
                #print("prompt attack success")
            ...
            #print(loss_fct(shift_logits_.view(-1, shift_logits_.size(-1))[:-length[i]], shift_labels_.view(-1)[:-length[i]]))
            #i = (i + 1)%10
            s = s + 1
            #if(s % 10 == 0):
                #writer.add_scalar("loss2",loss2,s)
                #print(l)
                #l = 0
            
            #print(loss_mid)
            
            #loss += torch.mean(loss_mid)
            #print(shift_logits_.view(-1, shift_logits_.size(-1)).shape)
            #print(shift_labels_.view(-1).shape)
...
step = [50, 100, 200, 300, 350, 400, 450, 500]
#
...
#raw_image = Image.open('./Result_syy/class5_init/10.jpg').convert('RGB')
#image = model.vis_processor(raw_image).unsqueeze(0).to(device)
#print(image)
#exit()
#image = model.vis_processor()
#image = torch.zeros(1, 3, 224, 224).to(device)
save_init_img_path = "./Result_syy/" + class_tag + "_init_img_" + str(seed) + ".png"
#image = torch.rand(3, 224, 224).to(device)
#print(image)
#save_init_img = (image.permute(1, 2, 0).cpu().numpy() * 255).astype(np.uint8)
#save_image(save_init_img, save_init_img_path)

#image = my_norm(image).unsqueeze(0)
#print(image)

image = torch.randn(1, 3, 224, 224).to(device)

image_s = denorm(image[0])
print(image_s)

save_init_img = (image_s[0].permute(1, 2, 0).cpu().numpy() * 255).astype(np.uint8)
save_image(save_init_img, save_init_img_path)
...
images = []
images.append(image)
adv_img = attack(images, model.shift_labels, step)
save_img_path = "Result_syy/" + class_tag + "_"+str(attack_power)+"_noise_"+str(attack_iters)+"_iters" + str(seed) + ".png"
...
image_emb, _ = model.model.encode_img(adv_img[0].unsqueeze(0))  # NOBUG
image_list = []
image_list.append(image_emb)
