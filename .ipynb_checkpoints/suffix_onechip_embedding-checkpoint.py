...
from torchattacks.attacks.pgd_embed import *
import os
import numpy as np
...
def parse_args():
    ...
    parser.add_argument("--attack_power", type=int, default="255")
    parser.add_argument(
        "--options",
        nargs="+",
...
#==============================================================#
Output_log_file_path = "./Result_new2/" + class_tag + "/output_" + str(attack_power) + '_'+ 'embed' + ".log"
...
'''
raw_image = Image.open('./Init_new2/S6/2_k.png').convert('RGB')
image = model.vis_processor(raw_image).unsqueeze(0).to(device)
image_emb, _ = model.model.encode_img(image)
print(torch.mean(image_emb))
print(torch.mean(torch.abs(image_emb)))
print(torch.min(image_emb))
exit()
'''


image = torch.zeros(1, 64, 4096).to(device)
# 保存为.png图像
...
image_emb = adv_img[0].half  # NOBUG
image_list = []
image_list.append(image_emb)
