...
def prompt_generate(model,  embedding, step=5000):
    ...
    for epoch in range(step):
        ...
        for p in range(model.train_num):
            ...
            #inp.append([i for i in range(model.train_num)])
            inp.append(p)


            loss = model.forward(inp)
            loss.backward()
            tot_loss += loss.item()
            actual_step += 1
...
#==============================================================#
Output_log_file_path = "./Result_new/" + class_tag + "/output" + "PEZ_un" + ".log"
...
class MiniGPT(nn.Module):
    def __init__(self, class_tag):
        ...
        for k in range(self.train_num):
            ...
            embs, inputs_tokens = self.get_context_emb(conv_, image_list)
    ...
    def forward(self, inp):
        ...
        if 1:
            ...
            embs, _ = self.get_context_emb(conv_, image_list)
    ...
    def forward2(self, inp):
        ...
        if 1:
            ...
            dataset = Dataset.from_list(embs)
            ...
            loss += loss_fct(shift_logits_.view(-1, shift_logits_.size(-1)), sl)
...
embeddings = prompt_generate(model, embeddings, step = 50)
...
for p in range(model.train_num):
    ...
    embs, _= model.get_context_emb(q_conv, image_list)
...
for p in range(model.test_num):
    ...
    embs, _ = model.get_context_emb(test_conv, image_list)
    print(embs.shape)
